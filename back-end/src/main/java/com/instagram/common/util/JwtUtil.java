...
public class JwtUtil {
    ...
    @Value("${jwt.secret}")
    private String secretKeyString; // 위조를 방지하기 위한 회사만의 비밀 도장 낙인
    ...
    /**
     * JWT 토큰 생성
     * 토큰의 형태는 ddahbakudwndaknas 이와 같은 형식으로 생성
     */
    public String generateToken(int userId, String userEmail) {
        Date now = new Date();                                      // 현재 시간
        Date validity = new Date(now.getTime() + expirationTime);   // 현재 시간 기준 + 24시간 유효기간 설정
